...
class hungarianMatcher(nn.Module):
    def __init__(self, cost_class: float = 1.0, cost_coord: float = 1.0, large_cost: float = 1e6):
        super().__init__()
    ...
    def forward(self, pred_logits, pred_points, gt_points, gt_mask):
        """
        pred_logits: [B, N, 2]  # 0=no-object, 1=object (未經 softmax)
        pred_points: [B, N, 2]
        gt_points:   [B, N, 2]
        gt_mask:     [B, N] bool
        """
        ...
        # softmax 機率
        prob = pred_logits.softmax(-1)  # [B,N,2]
        p_obj = prob[..., 1]            # [B,N]
        ...
        for b in range(B):
            ...
            # 分類成本：越像 object 成本越低
            if self.use_log:
                cost_class = -(p_obj[b].clamp_min(1e-8).log())[:, None]  # [N,1] -> broadcast [N,Mb]
            else:
                cost_class = -(p_obj[b])[:, None]                        # [N,1] -> broadcast [N,Mb]

            # 幾何成本（維持你原本的 L1 / L2 皆可）
            ...
            row, col = linear_sum_assignment(C_np)
...
class setCriterion(nn.Module):
    ...
    def forward(
        ...
        # 目標類別：0=背景，1=匹配到GT的pred
        B, N, _ = x0_hat.shape
        target_classes = torch.zeros((B, N), dtype=torch.long, device=cls_logits.device)
        target_classes[idx] = 1

        # (1) CE loss（跟 p2pnet / detr 一樣）
        # cross_entropy 期望 input: [B,C,N] + target: [B,N]
        L_exist = F.cross_entropy(
            cls_logits.transpose(1, 2),  # [B,2,N]
            target_classes,  # [B,N] long
            weight=self.empty_weight
        )

        # (2) soft count：用 object 機率
        prob_obj = cls_logits.softmax(-1)[..., 1]  # [B,N]
        pred_cnt = prob_obj.sum(dim=1)
        gt_cnt = mask.sum(dim=1).float()
        L_cnt = (pred_cnt - gt_cnt).abs() / (gt_cnt + 1.0)
        L_cnt = L_cnt.mean()

        # (3) 背景損失：把你原本用 sigmoid 的 prob_v 全改成 prob_obj
        pos_mask_bool = (target_classes == 1)
        ...
        for b in range(B):
            ...
            if tgt_idx.numel() == 0:
                ...
                pred_pix = m11_to_pixels(x0_hat[b], 256, 256)
                gt_pix = m11_to_pixels(p0[b, tgt_idx], 256, 256)
                dist = torch.cdist(pred_pix, gt_pix, p=2)
                dmin = dist.min(dim=1).values
                ignore = (dmin < r_pix)
                bgmask_b = (~pos_mask_bool[b]) & (~ignore)

            bg_losses.append(prob_obj[b][bgmask_b].mean() if bgmask_b.any() else prob_obj[b].new_tensor(0.0))
